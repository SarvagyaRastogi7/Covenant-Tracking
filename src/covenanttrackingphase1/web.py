...
<body>
  <div class="wrap">
    <h1>Upload your covenant workbook</h1>
    <p class="lede">drop an approved .xlsx file to compute DSCR and flag it either as compliant or breach.</p>
    <div class="card">
      <form id="f">
        <label class="drop" id="dropLabel">
          <input type="file" name="file" id="fileIn" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required />
          <div class="drop-inner">
            <strong>Choose or drag a spreadsheet</strong>
            <span>Financial_Statement &amp; Covenant_Config sheets · .xlsx only</span>
          </div>
        </label>
        <div class="file-name" id="fileMeta">No file selected</div>
        <div class="actions">
          <button type="submit" id="submitBtn">Run analysis</button>
          <span class="hint">Runs locally on this machine—nothing is sent to a third party.</span>
    ...
    <footer>Covenant tracking · deterministic pipeline</footer>
  </div>
  <script>
    ...
    function renderSummary(data) {
      ...
      if (data.report_type === 'COVENANT_DECISION_REPORT') {
        ...
        summaryEl.innerHTML =
          ...
          '<div><dt>Metric</dt><dd>' + escapeHtml(String(data.metric || '')) + '</dd></div>' +
          '<div><dt>Actual</dt><dd>' + escapeHtml(String(data.actual != null ? data.actual : '')) + '</dd></div>' +
          '<div><dt>Threshold</dt><dd>' + escapeHtml(String(data.threshold != null ? data.threshold : '')) + '</dd></div>' +
          '</dl>' +
          (cmp ? '<p class="summary-action"><strong>Comparison:</strong> ' + escapeHtml(cmp) + '</p>' : '') +
